...
namespace NeptuneEvo.Jobs
{
    class Miner : Script
    {
        ...
        {
            { 0, 10 },   // Уголь — 10$
            { 1, 25 },   // Железная руда — 25$
            { 2, 100 },  // Золотая руда — 100$
            { 3, 50 },   // Серная руда — 50$
            { 4, 250 },  // Изумруд — 250$
            { 5, 500 }   // Рубин — 500$
        };

        // Названия ресурсов (для логов/уведомлений)
        private static readonly Dictionary<int, string> OreNames = new Dictionary<int, string>()
        {
            { 0, "уголь" },
            { 1, "железо" },
            { 2, "золото" },
        ...
        // Максимальная вместимость «военного» склада
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                Main.CreateBlip(new Main.BlipData(527, "Гражданская шахта", new Vector3(-595.855, 2094.609, 131.4356), 4, true));
        ...
        public static void RemoteEvent_PlayerStartedMining(ExtPlayer player)
        {
            try
            {
                // 1) Получаем данные сессии и персонажа
                var sessionData = player.GetSessionData();
                if (sessionData == null) return;
                if (!player.IsCharacterData()) return;

                // 2) Админ-флаг: можно ли вообще майнить?
                if (!FunctionsAccess.IsWorking("PlayerStartedMining"))
                {
                    Notify.Send(
                        ...
                        LangFunc.GetText(LangType.Ru, DataName.FunctionOffByAdmins),
                        3000
                    );
                    return;
                }

                // 3) Если игрок уже в анимации или в деморгане — выходим без сообщений
                if (sessionData.AnimationUse != null || Main.IHaveDemorgan(player, true))
                    return;

                // 4) Запрещаем майнинг в наручниках
                if (sessionData.CuffedData.Cuffed)
                {
                    Notify.Send(
                        player,
                        NotifyType.Error,
                        NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.IsCuffed),
                        3000
                    );
                    return;
                }

                // 5) Запрещаем майнинг, если игрок «мертв»
                if (sessionData.DeathData.InDeath)
                {
                    Notify.Send(
                        ...
                        NotifyType.Error,
                        NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.IsDying),
                        3000
                    );
                    return;
                }

                // 6) Нельзя начать новую копку, если ещё не сдал предыдущую
                ...
                // 7) **Проверка наличия кирки** — если нет, уведомляем и выходим
                ItemStruct pickaxe = Chars.Repository.isItem(player, "inventory", ItemId.Pickaxe3)
                                 ?? Chars.Repository.isItem(player, "inventory", ItemId.Pickaxe2)
                                 ?? Chars.Repository.isItem(player, "inventory", ItemId.Pickaxe1);
                if (pickaxe == null)
                {
                    Notify.Send(
                        player,
                        NotifyType.Error,
                        NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouHaveNoPickaxe),
                        3000
                    );
                    return;
                }

                // 8) Обновляем индикатор уровня кирки на клиенте (1, 2 или 3)
                int toolLevel = pickaxe.Item.ItemId == ItemId.Pickaxe3 ? 3
                               : pickaxe.Item.ItemId == ItemId.Pickaxe2 ? 2
                               : 1;
                Trigger.ClientEvent(player, "mineJob_updateToolInfo", toolLevel);

                // 9) Запускаем анимацию майнинга и вешаем модель кирки
                ...
                // 10) Уведомляем клиент начать таймер копки
        ...
        public static void RemoteEvent_PlayerPutGovResources(ExtPlayer player)
        {
            try
            {
                ...
                // 1) Прекращаем любые текущие анимации «нести камень»
                Trigger.StopAnimation(player);
                Main.OffAntiAnim(player);
                Attachments.RemoveAttachment(player, Attachments.AttachmentsName.MineRock);

                // 2) Проверяем, действительно ли игрок несёт руду
                if (!MinerJobPlayersOreTypeInfo.ContainsKey(player.Value))
                    return;

                int oreType = MinerJobPlayersOreTypeInfo[player.Value];

                // 3) Проверяем, есть ли место на «военном» складе
                if (MineStockOres.Sum() >= MAX_CAPACITY)
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.SkladFulled), 3000);
                }
                else
                {
                    // 3.1) Добавляем 1 единицу руды на склад
                    MineStockOres[oreType]++;
                    SaveMineStocks(1);

                    // 3.2) Подсчитываем цену и выдаём деньги
                    int pricePerUnit = OrePricePerUnit[oreType];

                    // 3.2.1) Начисляем в characterData.Money и планируем запись в БД
                    Wallet.Change(player, pricePerUnit);

                    // 3.2.2) Логируем изменение в GameLog (гарантированная запись в таблицу Characters.Money)
                    GameLog.Money(
                        "server",
                        $"player({player.GetCharacterData().UUID})",
                        pricePerUnit,
                        $"sellGovOre({OreNames[oreType]})"
                    );

                    // 3.2.3) Обновляем HUD у клиента (слушайте событие "updateMoney" в вашем hud.js)
                    Trigger.ClientEvent(player, "updateMoney", player.GetCharacterData().Money);

                    // 3.3) Уведомляем об успешном начислении
                    Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter,
                        LangFunc.GetText(LangType.Ru, DataName.YouReceivedMoney, pricePerUnit), 3000);

                    // 3.4) Логируем сдачу руды в фракционный лог
                    Fractions.Table.Logs.Repository.AddLogs(
                        player,
                        FractionLogsType.TakeOre,
                        LangFunc.GetText(
                            LangType.Ru,
                            DataName.LoadedFromWarehouse,
                            $"{OreNames[oreType]} ×1"
                        )
                    );
                }

                // 4) Сбрасываем флаг «игрок больше не несёт руду»
                MinerJobPlayersOreTypeInfo.Remove(player.Value);

                // 5) Обновляем клиентский статус «несу/не несу»
                Trigger.ClientEvent(player, "mineJob_updateOreCarryStatus", false);

                // 6) Проигрываем короткую анимацию «положил» всем вокруг
                Trigger.ClientEventInRange(
                    player.Position,
                    250f,
                    "PlayAnimToKey",
                    player,
            ...
            {
                Log.Write($"RemoteEvent_PlayerPutGovResources Exception: {ex}");
            }
        }
