...
gm.events.add('client.inventory.GetItem', (jsonIds, toggled) => {
    try {
        const ids = JSON.parse(jsonIds); // массив ID предметов, которые есть
        // если есть хотя бы одна кирка и toggled == true
        const hasPickaxe = toggled && PickaxeItems.some(id => ids.includes(id));

        if (hasPickaxe) {
            // есть кирка — шлём серверу и он запустит всю анимацию/таймер
            mp.events.callRemote('PlayerStartedMining');
        } else {
            // кирки нет — показываем уведомление и НИЧЕГО БОЛЬШЕ не делаем
            mp.events.call('notify', 1, 9, "Чтобы копать, нужна кирка!", 3000);
        }
    } catch (e) {
        mp.events.callRemote("client_trycatch", "player/mine_job", "client.inventory.GetItem", e.toString());
    }
});
