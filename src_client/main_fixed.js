...
global.binderFunctions.onAutoPilot = () => {
    ...
    if (coord === null) {
        mp.events.call('notify', 1, 9, translateText("Сначала установи метку на карте."), 3000);
        return;
    }

    if (global.localplayer.vehicle === null) {
        mp.events.call('notify', 1, 9, translateText("Ты должен быть в машине, чтобы включить автопилот."), 3000);
        return;
    }

    global.localplayer.taskVehicleDriveToCoordLongrange(
        global.localplayer.vehicle.handle,
