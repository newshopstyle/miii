...
mp.keys.bind(global.Keys.VK_LEFT, true, function () {
    const key = testfsd[selecttestfsd];
    if (!key || !sitData.objectsInfo[key]) return;

    const objInfo = sitData.objectsInfo[key];
    if (objInfo.zOffset == null) objInfo.zOffset = 0;

    objInfo.zOffset -= 0.1;
    if (objInfo.zOffset < -5) objInfo.zOffset = 0;
});
...
mp.keys.bind(global.Keys.VK_RIGHT, true, function () {
    ...
    objInfo.zOffset += 0.1;
    if (objInfo.zOffset > 15) objInfo.zOffset = 0;
});
...
mp.keys.bind(global.Keys.VK_UP, true, function () {
    ...
    switch (selectsda) {
        case 1:
            objInfo.typeFlags = sitData.defaultData.yMinus;
            break;
        case 2:
            objInfo.typeFlags = sitData.defaultData.yPlus;
            break;
        case 3:
            objInfo.typeFlags = sitData.defaultData.xMinus;
            break;
        case 4:
            objInfo.typeFlags = sitData.defaultData.xPlus;
            break;
        default:
            selectsda = 0;
            delete objInfo.typeFlags;
            break;
    }
});
